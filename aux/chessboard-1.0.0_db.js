...
//
// Edits mostly for touch behaviour made by David Barry, 2021-02.
  ...
  function constructor (containerElOrString, config) {
    ...
    function beginDraggingPiece (source, piece, x, y) {
      ...
      $draggedPiece.attr('src', buildPieceImgSrc(piece)).css({
        ...
        left: 0,
        top: 0,
        transform: "translate(" + (x - squareSize / 2) + "px," + (y - squareSize/2) + "px)"
      })
      .addClass("dragged-piece")
    ...
    function touchstartSquare (evt) {
      ...
      if (isMovingByTouch) {
        ...
        if (!currentPosition.hasOwnProperty(square)) return
        if (isFunction(config.onDragStart) &&
            config.onDragStart(square, currentPosition[square], deepCopy(currentPosition), currentOrientation) === false) {
          return
        }
        ...
        draggedPiece = currentPosition[square]
    ...
    function mouseupWindow (evt) {
      ...
      // In Windows 11 Chrome, this function gets called on right-button mouseup
      // as well as left-button
    ...
    function addEvents () {
      ...
      if (isTouchDevice()) {
        $board
          .on('touchstart', '.' + CSS.square, touchstartSquare)
